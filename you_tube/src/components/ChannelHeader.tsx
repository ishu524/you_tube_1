            ...
            <div className="px-4 py-6 max-w-7xl mx-auto w-full">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1 space-y-2">
                        <h1 className="text-2xl md:text-4xl font-bold">{channel?.channelname || channel?.name || "Unknown Channel"}</h1>
                        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                            <span>@{((channel?.channelname || channel?.name || "user").toLowerCase()).replace(/\s+/g, "")}</span>
                        </div>
                        {channel?.description && (
